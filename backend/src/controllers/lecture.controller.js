...
//     isPublished: true,
//     processingStatus: "pending",  // becomes "completed" after transcript + quiz generation
//   });
...
/**
 * Fetches a lecture with its course populated, and verifies the requesting
 * user owns the course. Throws on any failure.
 */
...
export const getCourseLectures = asyncHandler(async (req, res) => {
  ...
  const lectures = await Lecture.find({ course: courseId, isPublished: true })
    .select("-__v")
    .sort({ order: 1 });
