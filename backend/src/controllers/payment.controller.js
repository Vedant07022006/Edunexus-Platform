...
export const createOrder = asyncHandler(async (req, res) => {
  ...
  if (course.isFree)      throw new ApiError(400, "This is a free course");
  if (!course.price || course.price <= 0) throw new ApiError(400, "Invalid course price");

  const instructorExists = await User.findById(course.instructor);
  if (!instructorExists)  throw new ApiError(400, "This course is no longer available for purchase");

  const existingEnrollment = await Enrollment.findOne({
    user: req.user._id, course: courseId, isActive: true,
  });
  if (existingEnrollment) throw new ApiError(400, "Already enrolled");
...
export const verifyPayment = asyncHandler(async (req, res) => {
  ...
  if (process.env.NODE_ENV !== "production" && razorpayPaymentId.startsWith("mock_")) {
    ...
    if (razorpayPaymentId.includes("mismatch")) rzpPayment.amount = 10;
  } else {
    try {
  ...
  const payment = await Payment.findOneAndUpdate(
    { razorpayOrderId, status: "pending", enrollmentCreated: false },
    { razorpayPaymentId, razorpaySignature, status: "completed", enrollmentCreated: true },
    { new: true }
  );
  ...
  if (!course || !course.instructor) throw new ApiError(400, "This course is no longer available");

  // Create enrollment
  const existingEnrollment = await Enrollment.findOne({ user: req.user._id, course: courseId });
  if (!existingEnrollment) {
    await Enrollment.create({ user: req.user._id, course: courseId, isActive: true, progress: 0 });
  ...
  // Send emails — student + instructor
  try {
    await Promise.all([
      ...
      sendEnrollmentEmail(req.user.email, {
        studentName: req.user.fullName,
        courseName:  course.title,
      }),
      sendInstructorEnrollmentEmail(course.instructor.email, {
...
export const getPaymentHistory = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(
    new ApiResponse(200, { payments, total: payments.length })
  );
});
...
export const getCoursePayments = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(
    new ApiResponse(200, { payments, total: payments.length, totalRevenue })
  );
});
...
export const razorpayWebhook = asyncHandler(async (req, res) => {
  ...
  if (event === "payment.captured" || event === "order.paid") {
    ...
    if (!paymentRecord) return res.status(404).send("Payment not found");
    if (paymentRecord.status === "completed") return res.status(200).send("Already processed");
    ...
    const webhookCourse = await Course.findById(courseId)
    ...
    if (!existingEnrollment) {
      await Enrollment.create({ user: userId, course: courseId, isActive: true, progress: 0 });
      await Course.findByIdAndUpdate(courseId, { $inc: { totalEnrollments: 1 } });
    ...
    try {
      ...
      await Promise.all([
        sendPaymentSuccessEmail(student.email, {
          studentName: student.fullName,
          courseName:  webhookCourse.title,
          amount:      paymentRecord.amount,
        }),
        sendEnrollmentEmail(student.email, {
          studentName: student.fullName,
          courseName:  webhookCourse.title,
        }),
        sendInstructorEnrollmentEmail(webhookCourse.instructor.email, {
          instructorName: webhookCourse.instructor.fullName,
          studentName:    student.fullName,
          courseName:     webhookCourse.title,
          amount:         paymentRecord.amount,
          enrolledAt:     new Date(),
