...
export const generateCourseAiAssist = (data)    => api.post('/courses/ai-assist', data); // NEW — Phase 2
...
export const updateLastWatchedPosition = (courseId, data) => api.patch(`/enrollments/position/${courseId}`, data); // NEW
...
// Payments
export const createOrder          = (courseId, data)      => api.post(`/payments/create-order/${courseId}`, data); // data NEW — Phase 4 coupon
...
export const generateSummary      = (lectureId) => api.post(`/transcripts/generate-summary/${lectureId}`); // NEW — Phase 2
...
export const getTranscriptForViewer = (lectureId) => api.get(`/transcripts/${lectureId}/view`); // NEW
...
export const generateWeakSpotReview = (attemptId)     => api.post(`/quiz-attempts/weak-spots/${attemptId}`); // NEW — Phase 2
...
export const submitReview         = (courseId, data)    => api.post(`/reviews/course/${courseId}`, data);
export const deleteReview         = (courseId)          => api.delete(`/reviews/course/${courseId}`);

// Discussions / Q&A (NEW — Phase 3)
...
// Instructor analytics (NEW — Phase 4)
...
// Reports / moderation (NEW — Phase 5)
