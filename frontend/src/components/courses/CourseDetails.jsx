...
export default function CourseDetails({ courseId }) {
  ...
  const [couponApplied, setCouponApplied] = useState(null); // { code, discountPercent, discountedPrice }
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // Pass the applied coupon (if any) so the backend discounts the order
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            ...
            // Show the real server error for easier debugging
