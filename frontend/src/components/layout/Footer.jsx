          ...
          <div className="md:col-span-2">
            ...
            <div className="flex gap-3 mt-6">
              {[Code2, Globe2, Share2].map((Icon, i) => (
                <a key={i} href="#" className="w-9 h-9 glass rounded-xl flex items-center justify-center text-slate-400 hover:text-white hover:bg-white/10 transition-all">
                  <Icon size={16} />
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {['Courses', 'Instructors', 'Pricing', 'Blog'].map(item => (
                <li key={item}>
                  <Link to="/courses" className="text-sm text-slate-400 hover:text-primary-400 transition-colors">{item}</Link>
                </li>
              ))}
            </ul>
          </div>
