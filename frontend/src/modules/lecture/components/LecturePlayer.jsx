...
import { getCourseLectures, updateProgress, updateLastWatchedPosition, checkEnrollment, checkQuizEligibility } from '../../shared/services/api.service';
...
export default function LecturePlayer({ courseId }) {
  ...
  const resumeLectureIdRef = useRef(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        resumeLectureIdRef.current = resumeId;
  ...
  useEffect(() => {
    ...
    if (lecturesData?.isInstructor) return;
