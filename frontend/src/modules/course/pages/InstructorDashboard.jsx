...
import { getMyCourses, getMyArchivedCourses, restoreCourse, getCourseEnrollments } from '../../shared/services/api.service';
import Navbar from '../../shared/components/Navbar';
import Button from '../../shared/components/Button';
import {
  BookOpen, Users, Plus, Eye, Settings,
  CheckCircle, Clock, ArrowRight, TrendingUp, Flag,
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="glass-strong rounded-2xl border border-slate-900/10 dark:border-white/10 w-full max-w-md p-6 shadow-2xl"
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-900/10 dark:hover:bg-white/10 transition-all"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <p className="text-lg font-semibold text-slate-900 dark:text-white">{student.user?.fullName || 'Unknown'}</p>
        ...
        <div className="glass rounded-xl p-3 border border-slate-900/[0.06] dark:border-white/[0.06] mb-4">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="glass rounded-xl p-3 border border-slate-900/[0.06] dark:border-white/[0.06]">
            <div className="flex items-center gap-1.5 mb-1">
              <Calendar size={12} className="text-primary-400" />
              <p className="text-xs text-slate-500">Enrolled on</p>
            </div>
            <p className="text-sm font-semibold text-slate-900 dark:text-white">{formatDate(student.createdAt)}</p>
          </div>
          ...
          <div className="glass rounded-xl p-3 border border-slate-900/[0.06] dark:border-white/[0.06]">
            ...
            <p className="text-sm font-semibold text-slate-900 dark:text-white">{student.progress || 0}%</p>
          </div>
    ...
    <>
      ...
      >
        <motion.div
          ...
          transition={{ duration: 0.2 }}
          onClick={(e) => e.stopPropagation()}
          className="rounded-2xl border border-slate-900/10 dark:border-white/10 w-full max-w-lg shadow-2xl overflow-hidden"
          style={{ background: '#13131f' }}
        >
          <div className="flex items-center justify-between p-5 border-b border-slate-900/[0.06] dark:border-white/[0.06]">
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Enrolled Students</h3>
            ...
            {loading ? (
              <div className="space-y-2">
                {Array(3).fill(0).map((_, i) => (
                  <div key={i} className="h-14 glass rounded-xl animate-pulse border border-slate-900/[0.06] dark:border-white/[0.06]" />
                ))}
              </div>
            ) : students.length === 0 ? (
              <div className="text-center py-12">
                <Users size={36} className="text-slate-600 mx-auto mb-3" />
                <p className="text-slate-600 dark:text-slate-400 text-sm font-medium">No students enrolled yet</p>
              ...
              <div className="space-y-2">
                {students.map((student, i) => (
                  <motion.button
                    ...
                    className="w-full flex items-center gap-3 p-3 glass rounded-xl border border-slate-900/[0.06] dark:border-white/[0.06]
                  ...
                  >
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className={`text-xs font-semibold ${
                        student.progress === 100 ? 'text-emerald-400' : 'text-slate-600 dark:text-slate-400'
                      }`}>
                        {student.progress || 0}%
                      </p>
      ...
      <div className="pt-20 pb-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            ...
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white mt-1">Hello, {user?.fullName} 👋</h1>
            <p className="text-slate-600 dark:text-slate-400 mt-1 text-sm">Manage your courses and track performance</p>
          </div>
        ...
        <div className="mb-8 flex flex-wrap gap-3">
          ...
          >
            ...
            Moderation Queue (Course Reports)
            <ArrowRight size={13} />
          </Link>
        </div>
          ...
          <div className="space-y-3">
            {courses.map((course, i) => (
              <motion.div
                ...
                className="glass rounded-2xl p-4 border border-slate-900/[0.06] dark:border-white/[0.06] flex items-center gap-4"
              >
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 glass rounded-xl text-xs
                               text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white border border-slate-900/[0.06] dark:border-white/[0.06]
                  ...
                  <Link to={`/courses/${course._id}`}>
                    <button className="p-2 glass rounded-xl text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all">
                      <Eye size={15} />
                    </button>
                  </Link>
      ...
      {!archivedLoading && archivedCourses.length > 0 && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
          <button
            ...
            className="w-full flex items-center justify-between p-4 glass rounded-2xl border border-slate-900/[0.06] dark:border-white/[0.06] hover:border-slate-900/10 dark:hover:border-white/10 transition-all"
          >
            <span className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-300">
              <Archive size={15} className="text-slate-500" />
              Archived Courses
              <span className="text-xs bg-slate-900/5 dark:bg-white/5 text-slate-500 px-2 py-0.5 rounded-full">{archivedCourses.length}</span>
            </span>
            <ChevronDown size={16} className={`text-slate-600 dark:text-slate-400 transition-transform ${showArchived ? 'rotate-180' : ''}`} />
          </button>
              ...
              >
                <div className="space-y-3 mt-3">
                  {archivedCourses.map((course) => (
                    <div
                      key={course._id}
                      className="glass rounded-2xl p-4 border border-slate-900/[0.06] dark:border-white/[0.06] flex items-center gap-4 opacity-75"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-slate-700 dark:text-slate-300 text-sm truncate">{course.title}</h3>
                        <div className="flex items-center gap-3 mt-1 text-xs text-slate-500">
                          <span className="flex items-center gap-1"><BookOpen size={11} />{course.totalLectures} lectures</span>
                      ...
                      <span className="text-xs px-2.5 py-1 rounded-full bg-slate-500/20 text-slate-600 dark:text-slate-400 flex-shrink-0">
                        Discontinued
                      </span>
