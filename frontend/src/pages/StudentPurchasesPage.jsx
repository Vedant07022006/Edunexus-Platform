    ...
    <div className="min-h-screen">
      ...
      <div className="pt-20 pb-12 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mt-1">My Purchases</h1>
          <p className="text-slate-400 mt-1 text-sm">All your enrolled courses and payment history</p>
        </motion.div>
            ...
            {FILTERS.map((f) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-all ${
                  filter === f ? 'gradient-primary text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                {f}
              </button>
        ...
        {loading ? (
          <div className="space-y-4">
            {Array(3).fill(0).map((_, i) => (
              <div key={i} className="glass rounded-2xl h-32 animate-pulse border border-white/[0.06]" />
            ))}
          </div>
            ...
            {enrollments.map((enrollment, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="glass rounded-2xl border border-white/[0.06] p-4 flex gap-4"
                >
                  {/* Thumbnail */}
                    ...
                    <div className="mt-3 flex items-center justify-between flex-wrap gap-3">
                      <div className="flex gap-5 text-xs">
                        <div>
                          ...
                          <p className="text-slate-300 font-medium mt-0.5">{formatDate(enrollment.createdAt)}</p>
                        </div>
                        <div>
                          <p className="text-slate-500">Amount paid</p>
                          <p className={`font-medium mt-0.5 ${course.isFree ? 'text-emerald-400' : 'text-slate-300'}`}>
